...
def test_decode_open_order_account_layout():
    ...
    with open(OPEN_ORDER_ACCOUNT_BIN_PATH, "r") as input_file:
        ...
        assert Pubkey(open_order_account.market) == Pubkey.from_string("4r5Bw3HxmxAzPQ2ATUvgF2nFe3B6G1Z2Nq2Nwu77wWc2")
        assert Pubkey(open_order_account.owner) == Pubkey.from_string("7hJx7QMiVfjZSSADQ18oNKzqifJPMu18djYLkh4aYh5Q")
...
def test_decode_open_order_account():
    ...
    with open(OPEN_ORDER_ACCOUNT_BIN_PATH, "r") as input_file:
        base64_res = input_file.read()
        data = base64.decodebytes(base64_res.encode("ascii"))
        open_order_account = OpenOrdersAccount.from_bytes(Pubkey(1), data)
